   ...
   public function create() {
       // mysql_create_db($this->name);
        ...
        $this->query('CREATE TABLE category (categoryID  int NOT NULL AUTO_INCREMENT, name char(30),  link_image char(255), description char(255), PRIMARY KEY(categoryID))');
		  ...
		  Category::add("Nabiał", "uploads/Nabial.jpg" , "Produkty wyrobu mlecznego");
		  Category::add("Warzywa", "uploads/warzywa.jpeg" , "Produkty rolnicze");
                  ...
                  Category::add("Mięso", "uploads/mieso.jpeg" , "Produkty pochodzenia zwierzecego");
		...
		$result = DB::getInstance()->query("SELECT * FROM productComponent LEFT JOIN component ON component.componentshopID =  productComponent.componentshopID WHERE product = " .$productID);
		 ...
		 while($row = mysql_fetch_array($result)) {
		   ...
		   $lisView .= ' <h2>'.$row["name"].'</h2>';
			...
			$lisView .= '<a href="product_edit.php?Product='.$productID.'&deleteComponent=' .$row["componentshopID"] .'" class="ui-btn ui-icon-delete ui-btn-icon-left">Usu�</a>';
		 ...
		 // if (!file_exists($row["link_image"])) $row["link_image"] = "images/default-no-image.png";
		   ...
		   $lisView .=    ' </a>';
			...
			if ($row['status'] == 0) {
			 	...
			 	$lisView .= ' <a href="index.php?archiveShopList='.$row["shopListID"].'" data-transition="pop" data-icon="edit">Archwizuj listę zakupów</a>';
			}
	...
	public static function getListCategory() {
		 ...
		 while($row = mysql_fetch_array($result)) {
		  if (!file_exists($row["link_image"])) $row["link_image"] = "images/default-no-image.png";
		   ...
		   $lisView .=    ' </a>';
		 	$lisView .=     ' <a href="product.php?deleteProduct='.$row["productID"].'" data-transition="pop" data-icon="delete">Usu� produkt</a>';
		 	$lisView .= '</li>';
        }
		 ...
		 while($row = mysql_fetch_array($result)) {
		  if (!file_exists($row["link_image"])) $row["link_image"] = "images/default-no-image.png";
		   ...
		   $lisView .=    ' <h2>'.$row["name"].'</h2>';
		  	$lisView .=     ' <p>'.$row["description"].'</p>';
		   $lisView .=    ' </a>';
		 	$lisView .=     ' <a href="category.php?deleteCategory='.$row["categoryID"].'" data-transition="pop" data-icon="delete">Usu� kategori�</a>';
		 	$lisView .= '</li>';
        }

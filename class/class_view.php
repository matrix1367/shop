...
class View {
    ...
    public function getNavigationBars($role) {
        ...
        if ($role == 1) {
            ...
            $bar .= '<li><a href="component.php">Składniki</a></li>';
    ...
    public static function getGrupsProductArchive($listShopID) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            $view .= ' <h2>' . $row["name"] . '</h2>';
            $view .= ' <p> Ilość: ' . $row["amount"] . ' Cena: ' . $row["price"] . ' zł</p>';
            $view .= ' </a>';
            //	 	$view .=     ' <a href="product.php?deleteProduct='.$row["productID"].'" data-transition="pop" data-icon="delete">Usu� produkt</a>';
            $view .= '</li>';
        }
        $view .= "</ul></div>";
        return $view;
    ...
    /*
      ...
      $view .= '</div><a href="schop_archive.php" class="ui-btn">Powr�t</a></form>';
      return $view;
      }
     */
    ...
    public function viewBuyList($ShopListID) {
        ...
        while ($row = mysql_fetch_array($result)) {
		  ...
		  <img  width="60px" height="60px" src="' . $row["link_image"] . '" /></td> </label>';
            $listView .= ' <td> <label for="amount">Ilość: </label> <input  type="text" name="amount[]" id="amount"> </td>  ';
            $listView .= ' <td><label for="price">Cena: </label> <input  type="text" name="price[]" id="price"></td></tr> ';
        }
        $listView .= '    </tbody></table>';
        $listView .=' <input type="submit" value="Zakończ zakupy" /></form>';
        //	 $listView .= ' </ul></div>';
        return $listView;
    }
